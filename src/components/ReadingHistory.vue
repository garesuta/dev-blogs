  ...
  <div class="reading-history-section">
    ...
    >
      ...
      Please <a
        href="/login"
        class="alert-link"
      >log in</a> to view your reading history.
    </div>
    ...
    <div v-else>
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        >
          <i class="bi bi-trash me-1" /> Clear History
        </button>
      </div>
      ...
      <div class="list-group list-group-flush shadow-sm rounded border-0">
        ...
        >
          <div
            v-if="getArticleById(entry.articleId)"
          ...
          >
            <div class="flex-grow-1">
              ...
              <div class="d-flex gap-2 align-items-center">
                <span
                  v-if="getArticleById(entry.articleId)!.category"
                  class="badge bg-info text-white small"
                >
                  {{ getArticleById(entry.articleId)!.category }}
                </span>
                <span
                  v-if="getArticleById(entry.articleId)!.difficulty"
                  :class="['badge', 'text-white', 'small', 'bg-' + (getArticleById(entry.articleId)!.difficulty === 'beginner' ? 'success' : getArticleById(entry.articleId)!.difficulty === 'intermediate' ? 'warning' : 'danger')]"
                >
                  {{ getArticleById(entry.articleId)!.difficulty }}
                </span>
                <span class="text-muted small">
                  <i class="bi bi-clock me-1" /> {{ formatDate(entry.readAt) }}
                </span>
              </div>
            </div>
