...
```bash
python scripts/init_skill.py <claude-skill-name> --path .claude/skills/<claude-skill-name>
```
...
```bash
# Run w/ test fixtures
python scripts/<script>.py tests/<claude-skill>/fixtures/input.txt
```
...
diff output.txt tests/<claude-skill>/expected/output.txt
```
...
```bash
# Package & validate claude-skill
python scripts/package_skill.py .claude/skills/<claude-skill-name>
```
...
Create `tests/claude-skill/<skill>/test_skill.py`:
...
description: Brief desc. Use third-person ("This claude-skill should be used when...")
...
---
Ref: `.claude/skills/skill-creator/SKILL.md`, `.claude/skills/token-formatter/SKILL.md`,
`.claude/skills/document-skills/md/SKILL.md`
