---
name: plan-feature
description: Production-grade feature planning with dual-AI validation (Claude + Cline CLI subagent)
tools:
  - AskUserQuestion
  - Bash(cline *)
  - Read
...
### Step 2: Cline CLI Validation (External - 2 Calls)

Only if Claude checks pass.

**Pre-flight: Verify Cline CLI model configuration**
```bash
# Check current provider & model from config
cat ~/.cline/data/globalState.json | grep -E '"apiProvider"|"apiModelId"'

# Or open the interactive config panel to review all settings
cline config

# To use a specific model for validation calls (override default)
cline -m <model-id> -y -p "..."

# To switch provider/model permanently
cline auth -p <provider> -k <api-key> -m <model-id>
# Example: switch to Gemini for dual-AI validation
cline auth -p gemini -k <GEMINI_API_KEY> -m gemini-2.5-pro
```

**Call 1 - Security + SRE:**
```bash
cline -y -p "Act as Security Engineer + SRE. Review for:
...
```bash
cline -y -p "This feature failed 6 months from now.
...
> **Note:** `cline -y` runs in headless YOLO mode (auto-approve, no interactive UI). Use `cline -m <model>` to override the default model per-call, or run `cline auth` to switch provider permanently. For true dual-AI validation, configure Cline CLI to use a different provider than Claude (e.g., `cline auth -p gemini -m gemini-2.5-pro`). Always verify with `cat ~/.cline/data/globalState.json | grep -E '"apiProvider"|"apiModelId"'` before running validation.
...
- [ ] Cline CLI Call 1: No Critical security issues
- [ ] Cline CLI Call 2: Rating >= 8/10, top failures addressed
- [ ] **Plan saved to `docs/YYYY/MM/[feature].md`**
...
DISCOVER → RESEARCH → PLAN → VALIDATE → REFINE
 ...
 questions            template   Cline CLI until 8+
```
...
# Pattern: docs/YYYY/MM/feature-name.md
FILENAME="docs/plan/$(date +%Y)/$(date +%m)/$(echo "[FEATURE_NAME]" | tr '[:upper:]' '[:lower:]' | tr ' ' '-').md"

# Write plan with metadata header
cat > "$FILENAME" << 'EOF'
---
created: $(date +%Y-%m-%d)
status: approved
validation_score: [CLINE_CLI_SCORE]/10
author: [USER]
---
...
| Run 4 Cline CLI calls | Consolidate to 2 targeted calls |
...
---
Integrates: cline-cli, context7, AskUserQuestion | ~700 tokens (was ~1100)

## Cline CLI Quick Reference

| Command | Purpose |
|---------|---------|
| `cat ~/.cline/data/globalState.json \| grep -E '"apiProvider"\|"apiModelId"'` | Check current provider & model |
| `cline config` | Open interactive config panel (Settings, Rules, Workflows, Hooks) |
| `cline auth` | Interactive auth wizard (set provider, key, model) |
| `cline auth -p <provider> -k <key> -m <model>` | Quick auth setup in one line |
| `cline -m <model-id> "prompt"` | Run task with a specific model (one-off override) |
| `cline -y "prompt"` | Headless YOLO mode (auto-approve, plain text output) |
| `cline -y --json "prompt"` | Headless + JSON output (for scripting/parsing) |
| `cline -p "prompt"` | Start in Plan mode (analyze before acting) |
| `cline version` | Show installed CLI version |
