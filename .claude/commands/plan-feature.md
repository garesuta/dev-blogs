...
**IMPORTANT: This command ONLY runs when explicitly invoked via `/plan-feature`. Creates production-grade implementation
plans with multi-source validation.**
...
**IMPORTANT: Use `docs-navigator` skill FIRST before Grep/Glob.**

Before planning:

1. **Consult docs-navigator** - Use `.claude/skills/docs-navigator/SKILL.md` to find relevant docs
2. `Read` - Read matched docs for architecture context
3. `Grep`/`Glob` - Only for specific patterns not in docs
4. Check for potential circular dependencies

> Skill: `.claude/skills/docs-navigator/` - Maps topics → doc locations
