...
try:
    ...
    print(
        "Note: Install 'mistune' for better validation: pip install mistune",
        file=sys.stderr,
    )
...
def main():
    ...
    print(f"\n{'=' * 50}")
    print(
        f"Files: {len(files)} | Issues: {len(all_issues)} ({errors} errors, {warnings} warnings, {infos} info)"
    )
