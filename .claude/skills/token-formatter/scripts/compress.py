...
def compress_text(text, level=2):
    ...
    if level >= 2:
        ...
        abbrevs = {
            ...
            r"\bargument\b": "arg",
            r"\bvariable\b": "var",
            r"\binterface\b": "iface",
            r"\bimplementation\b": "impl",
            r"\bsynchronous\b": "sync",
...
def main():
    parser = argparse.ArgumentParser(
        description="Compress text for LLM context optimization."
    )
    parser.add_argument(
        "file",
        nargs="?",
        help="Input file path (optional, reads from stdin if not provided)",
    )
    parser.add_argument(
        "--level",
        type=int,
        choices=[1, 2],
        default=2,
        help="Compression level (1=Light, 2=Medium/Heavy)",
    )
