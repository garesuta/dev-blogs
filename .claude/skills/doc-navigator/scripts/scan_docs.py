...
def scan_project(project_path: Path) -> dict:
    ...
    if doc_root:
        ...
        results["all_docs"] = [str(f.relative_to(project_path)) for f in all_docs]

    # Also check root for standalone docs
    root_docs = [
        f for f in project_path.iterdir() if f.is_file() and f.suffix.lower() == ".md"
    ]
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show all documentation files"
    )
